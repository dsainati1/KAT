...
let rec collect_states (vars : StringSet.t) (acc : term list) (e : term) : term list = 
	...
	List.iter (fun e -> print_string (term_to_string e ^ ", ")) filtered;
	print_endline "";
...
let print_idset (ids : idset) = 
	print_string "{";
	List.iter (fun i -> print_string (i ^ ", ")) ids;
	print_string "}"

let print_idset_list (ids : idset list) = 
	print_string "[";
	List.iter (fun i -> print_idset i; print_string ", ") ids;
	print_string "]"

let print_transition ((src, l, dest) : (id * string * id)) = 
	print_endline (src ^ " -> " ^ dest ^ " on " ^ l)

let print_d_transition (src : id) ((l, dest) : d_transition) = 
	print_endline (src ^ " -> " ^ dest ^ " on " ^ l)
...
let deterministic_transitions (state : n_state) : (id * string * id) list =
	let pre_transition (id : id) ((l : string), (targets : id list)) : (id * string * id) list = 
		List.map (Tuple3.make id l) targets in
...
let determinize ((start, states) : nfa) : dfa = 
	...
	let build_d_states (transitions : (id * string * id) list) : d_state list = 
		...
		let build_d_state (id : id list) : d_state = 
			...
			List.filter (Tuple3.first %> flip List.mem id) transitions 
			|> List.map Tuple3.get23 
			|> List.fold_left (flip destination_union) [] 
			|> List.map (Tuple2.map2 name_of_dstate)
			|> make_state in

        collect_d_state_ids start states
        |> List.map build_d_state in 
